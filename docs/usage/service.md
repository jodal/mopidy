...
On Debian systems (both those using systemd and not) you can enable the
Mopidy service by running:
...
When using PulseAudio, you will typically have a PulseAudio server run
by your main user. Since Mopidy as a system service is running as its
own user, it can't access your PulseAudio server directly. Running
PulseAudio as a system-wide daemon is [discouraged by
upstream](https://www.freedesktop.org/wiki/Software/PulseAudio/Documentation/User/WhatIsWrongWithSystemWide/).
Rather you can configure PulseAudio and Mopidy so that Mopidy sends the audio to
the PulseAudio server already running as your main user.
...
``` title="~mopidy/.pulse/client.conf"
default-server=127.0.0.1
```

## System service and PipeWire

When using PipeWire, you'll encounter a situation analogous to the one described
for PulseAudio. You must configure PipeWire and Mopidy so that Mopidy sends the
audio to the PipeWire server already running as your main user.
...
``` title="pipewire-pulse.conf" hl_lines="6"
pulse.properties = {
