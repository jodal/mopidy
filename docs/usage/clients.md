...
There are many clients available that use `ext-http` (TODO) to control Mopidy.

### Web extensions

Mopidy extensions can make additional web APIs available through
Mopidy's builtin web server by implementing the `http-server-api` (TODO). Web
clients can use the `http-api` (TODO) to control Mopidy from JavaScript.
...
There are several web based MPD clients, which doesn't use the `ext-http` (TODO)
frontend at all, but connect to Mopidy through the [mopidy-mpd][mopidy-mpd]
