...
class TestLoadExtensions:
    ...
    def test_load_extensions(self, mock_entry_point):
        ...
        assert load_extensions() == [expected]
    ...
    def test_load_extensions_real(self):
        installed_extensions = load_extensions()
...
class TestValidateExtensionData:
    ...
    def test_real(self):
        for dist in load_extensions():
            assert validate_extension_data(dist)

    def test_ok(self, ext_data):
        assert validate_extension_data(ext_data)

    def test_name_mismatch(self, ext_data):
        ext_data.entry_point.name = "barfoo"
        assert not validate_extension_data(ext_data)
    ...
    def test_get_cache_dir(self, ext_data):
        ...
        with mock.patch.object(mopidy.ext._extension.paths, "get_or_create_dir"):
