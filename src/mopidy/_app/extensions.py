...
class ExtensionRecord:
    ...
    def _load_extension(
        ...
        if not (config_defaults := extension.get_default_config()):
            logger.error(
                f"Extension {extension.ext_name} does not have a default config"
            )
            return stopped_record
    ...
    def check_config_and_env(self, config: ConfigManager) -> None:
        if self.status == ExtensionStatus.STOPPED_BY_LOAD_ERROR:
            return  # Skip config checks until load errors are resolved.

        if not config.config[self.ext_name]["enabled"]:
            self.status = ExtensionStatus.DISABLED
            config.disable_extension(self.ext_name, "Extension disabled by user config")
            return

        if config.errors.get(self.ext_name):
            self.status = ExtensionStatus.STOPPED_BY_CONFIG_ERROR
            config.disable_extension(
                self.ext_name, "Extension stopped due to config errors"
            )
            return

        if not self._self_check():
            self.status = ExtensionStatus.STOPPED_BY_SELF_CHECK
            config.disable_extension(self.ext_name, "Extension stopped by self-check")
            return
...
class ExtensionManager(UserDict[str, ExtensionRecord]):
    ...
    def check_config_and_env(self, config: ConfigManager) -> None:
        for record in self.data.values():
            record.check_config_and_env(config)
